...
public:
   ...
   void addBuffer(const char* data, const int& len, int nttl, bool border, unsigned int unregion);
...
private:
   ...
   struct Block
   {
	  ...
	  unsigned int m_unRegion;                //丢包区域控制
	  int m_iMsgCount;
   ...
   struct Buffer
   {
	  ...
	  Block* m_pBufBlock;		//dvr中为减少new次数，引入该值进行一次性分配
      int m_iSize;			// size
