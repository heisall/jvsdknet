...
const int CPacket::m_iPktHdrSize = 12;//16;
...
m_iTimeStamp((int32_t&)(m_nTimeStampTemp)),//m_iTimeStamp((int32_t&)(m_nHeader[2])),
m_iID((int32_t&)(m_nHeader[2])),//m_iID((int32_t&)(m_nHeader[3])),
m_pcData((char*&)(m_PacketVector[2].iov_base)),
__pad()
{
	for (int i = 0; i < 3; ++ i)
	{
   ...
   m_PacketVector[1].iov_base = (char *)m_nHeader;
   m_PacketVector[1].iov_len = CPacket::m_iPktHdrSize;
}
...
{
   return m_PacketVector[2].iov_len;
}

void CPacket::setLength(const int& len)
{
   m_PacketVector[2].iov_len = len;
}
...
void CPacket::pack(const int& pkttype, void* lparam, void* rparam, const int& size)
{
   ...
   case 2: //0010 - Acknowledgement (ACK)
      ...
      m_PacketVector[2].iov_base = (char *)rparam;
      m_PacketVector[2].iov_len = size;
   ...
   case 6: //0110 - Acknowledgement of Acknowledgement (ACK-2)
      ...
      m_PacketVector[2].iov_base = (char *)&__pad; //NULL;
      m_PacketVector[2].iov_len = 4; //0;
   ...
   case 0: //0000 - Handshake
      ...
      m_PacketVector[2].iov_base = (char *)rparam;
      m_PacketVector[2].iov_len = size; //sizeof(CHandShake);
   ...
   case 65535: //0x7FFF - Reserved for user defined control packets
      ...
      {
         m_PacketVector[2].iov_base = (char *)&__pad;
         m_PacketVector[2].iov_len = 4;
      }
...
{
   // read [1] bit 0~1
   return ((m_nHeader[1] >> 30) & 0x00000003);
}
...
CPacket* CPacket::clone() const
{
   ...
   pkt->m_pcData = new char[m_PacketVector[2].iov_len];
   memcpy(pkt->m_pcData, m_pcData, m_PacketVector[2].iov_len);
   pkt->m_PacketVector[2].iov_len = m_PacketVector[2].iov_len;
